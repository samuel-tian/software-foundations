...
Fixpoint alternate (l1 l2 : natlist) : natlist :=
  ...
  | m1 :: t1, [] => l1
  | [], m2 :: t2 => l2
  | m1 :: t1, m2 :: t2 => m1 :: m2 :: (alternate t1 t2)                         
  end.
...
Fixpoint subset (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | nil => true
  | m :: t => match (member m s2) with
              | true => subset t (remove_one m s2)
              | false => false
              end
  end.
...
Theorem bag_count_sum: forall (n : nat) (s1 s2 : bag), count n (s1 ++ s2) = (count n s1) + (count n s2).
Proof.
  intros n s1 s2. induction s1 as [| k s1' IHs1'].
  { reflexivity. }
  { simpl. destruct (k =? n) as [|].
...
Proof.
  intros l default. destruct l as [| m t].
  { reflexivity. }
  { reflexivity. }
Qed.
