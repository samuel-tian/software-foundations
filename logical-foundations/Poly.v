...
Theorem app_nil_r : forall (X:Type), forall l:list X,
  l ++ [] = l.
Proof.
  intros X l. induction l as [| m l' IHl'].
  { reflexivity. }
...
Proof.
  intros X l1 l2. induction l1 as [| t l1' IHl1'].
  { reflexivity. }
...
Theorem rev_involutive : forall X : Type, forall l : list X,
  rev (rev l) = l.
Proof.
  intros X l. induction l as [| t l' IHl'].
  { reflexivity. }
...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
  match l with
  | [] => ([], [])
  | m :: t => ((fst m) :: fst (split t), (snd m) :: snd (split t))
  end.
...
Lemma map_distr : forall (X Y: Type) (f : X -> Y) (l1 l2 : list X), map f (l1 ++ l2) = (map f l1) ++ (map f l2).
Proof.
  intros X Y f l1 l2. induction l1 as [| m l1' IHl1'].
  { reflexivity. }
  { simpl. rewrite -> IHl1'. reflexivity. }
Qed.
...
Proof.
  intros X Y f l. induction l as [| m l' IHl'].
  { reflexivity. }
  { simpl. rewrite -> map_distr. rewrite -> IHl'. reflexivity. }
Qed.
